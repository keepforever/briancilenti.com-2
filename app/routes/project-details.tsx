...
export default function ProjectDetails({ loaderData }: Route.ComponentProps) {
  const { title, imageSrc, githubUrl, description, deployedUrl, demoUrl } = loaderData
  ...
  if (!title) {
    return (
      <div className="flex flex-col items-start rounded-md bg-slate-600 p-6">
        <Link to="/projects" className="mb-4 text-2xl font-bold text-blue-300 hover:text-blue-700">
          Back to Projects
        </Link>
        <div className="text-3xl text-red-500 sm:text-6xl">Project not found</div>
      </div>
    )
  }
  ...
  return (
    <div className="m-2 flex flex-col justify-between gap-4 rounded-lg bg-gray-800 p-6 shadow-md">
      ...
      <img src={imageSrc} alt={title} className="object-fit w-full rounded-md" />
